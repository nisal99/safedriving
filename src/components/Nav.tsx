  ...
  return (
    <>
      <header className="sticky top-0 z-30 border-b border-line bg-surface/90 pt-[env(safe-area-inset-top)] backdrop-blur">
        <div className="mx-auto flex h-14 max-w-5xl items-center gap-3 px-[max(1rem,env(safe-area-inset-left))] sm:px-6">
          <Link href="/" className="flex min-w-0 items-center gap-2 font-bold">
            ...
            <span className="truncate whitespace-nowrap text-[15px] leading-tight sm:text-base">
              Seoul Driving Test <span className="hidden text-muted lg:inline">· Practice</span>
            </span>
          </Link>
          <nav className="ml-auto hidden items-center gap-0.5 md:flex lg:gap-1" aria-label="Main">
            {LINKS.slice(1).map((l) => (
              <Link
                key={l.href}
                href={l.href}
                className={`whitespace-nowrap rounded-lg px-2.5 py-2 text-sm font-medium lg:px-3 ${active(l.href) ? "bg-brand-soft text-brand" : "text-muted hover:text-ink"}`}
              >
                {l.label}
                {l.href === "/wrong" && wrong.length > 0 && <span className="ml-1.5 chip bg-bad-soft text-bad">{wrong.length}</span>}
              </Link>
            ))}
            <Link href="/settings" className={`rounded-lg px-2.5 py-2 text-sm font-medium lg:px-3 ${active("/settings") ? "bg-brand-soft text-brand" : "text-muted hover:text-ink"}`}>
              Settings
            </Link>
            <InstallButton />
          </nav>
          <div className="ml-auto flex shrink-0 items-center md:hidden">
            ...
            <Link href="/settings" className="grid h-10 w-10 place-items-center rounded-full text-muted hover:bg-surface-2" aria-label="Settings">
